 ...
 * in the code.
 *
 * In order to handle the RS485 serial port, the driver has been modified to increase
 * logic on controller.
 *
 * Everything is line up with generic kernel framework and standard UNIX API.
 *
 */


#if defined(CONFIG_SERIAL_XILINX_PS_UART_CONSOLE)
#undef CONFIG_SERIAL_XILINX_PS_UART_CONSOLE
#endif
...
#define CDNS_UART_NAME		"xuartpsr18"
...
#define CDNS_UART_REFSPEED	0x1250000
#define CDNS_UART_REFDIV	4
#define CDNS_UART_TXRX		1
#define CDNS_UART_TXONLY	2
#define CDNS_UART_RXONLY	3
#define CDNS_UART_FLOWCTL	1
...
#define cnds_mode_delay(_x)	for(i=0;i<=_x;i++)
#define cdns_uart_ctl(_p,_c)	gpio_set_value(_p,_c)
#define to_cdns_uart(_nb) container_of(_nb, struct cdns_uart, \
		clk_rate_change_nb)


static void cdns_uart_poll_put_char(struct uart_port *port, unsigned char c);
static void cdns_uart_enable_wr(struct cdns_uart *p)
{

	if(!p)
	   return;

	cdns_uart_ctl(p->pin_rx,CDNS_UART_FLOWCTL);
	cdns_uart_ctl(p->pin_tx,CDNS_UART_FLOWCTL);
}

static void cdns_uart_enable_rd(struct cdns_uart *p)
{
	...
	ref = (p->clk_rate - p->baud)/CDNS_UART_REFDIV ;
	cnds_mode_delay(ref);
	cdns_uart_ctl(p->pin_rx,(CDNS_UART_FLOWCTL & 0x0));
	cdns_uart_ctl(p->pin_tx,(CDNS_UART_FLOWCTL & 0x0));
}
...
static void cdns_uart_handle_tx_(void *dev_id)
{
	struct uart_port *port = (struct uart_port *)dev_id;
	struct cdns_uart *cdns_uart = port->private_data;
	...
	if(cdns_uart->type == CDNS_UART_RXONLY)
		return;

	cdns_uart_enable_wr(cdns_uart);
	numbytes = port->fifosize;
	disable_irq(port->irq);
//	writel(CDNS_UART_IXR_TXEMPTY, port->membase + CDNS_UART_ISR);

	while (numbytes && !uart_circ_empty(&port->state->xmit)) {
		 cdns_uart_poll_put_char(port,port->state->xmit.buf[port->state->xmit.tail]);
		 port->icount.tx++;
	...
	if(cdns_uart->type == CDNS_UART_TXONLY)
		return;

	cdns_uart_enable_rd(cdns_uart);
}
/**
...
static const struct of_device_id cdns_uart_of_match[] = {
	{ .compatible = "xlnx,uartps", },
	{ .compatible = "cdns,uart-r1p18", },
	{ .compatible = "cdns,uart-r1p19", },
	{}
};
...
static int cdns_uart_probe(struct platform_device *pdev)
{
	...
	cdns_uart_data->txempty = 0x0; //Clean the txempty flag
	cdns_uart_data->type = 0x0; //Clean chtype

	if (of_property_read_u32(pdev->dev.of_node,"channel-reg",&cdns_uart_data->type)){
		printk(KERN_EMERG "UART:%d, : Using default channel type 1 \n",cdns_uart_data->id);
		cdns_uart_data->type = CDNS_UART_TXRX;
	} else {
		if( (cdns_uart_data->type != CDNS_UART_TXRX) && \
				(cdns_uart_data->type != CDNS_UART_TXONLY) && \
				(cdns_uart_data->type != CDNS_UART_RXONLY)){
			printk(KERN_EMERG "UART:%d, Invalid channel type, using default type 1 \n",cdns_uart_data->id);
			cdns_uart_data->type = CDNS_UART_TXRX;
		}
	}

	if (of_property_read_u32(pdev->dev.of_node,"pin_tx",&cdns_uart_data->pin_tx)){
		printk(KERN_EMERG "UART:%d, : No pin_tx defined in dts\n",cdns_uart_data->id);
	} else {
		sprintf(gname,"uart%d-pin-tx",cdns_uart_data->id);
		if (!gpio_is_valid(cdns_uart_data->pin_tx)){
			printk(KERN_EMERG "Invaild TX pin \n");
		} else {
			gpio_request(cdns_uart_data->pin_tx,gname);
			if (cdns_uart_data->type == CDNS_UART_TXONLY) {
				gpio_direction_output(cdns_uart_data->pin_tx,1);
			} else {
				gpio_direction_output(cdns_uart_data->pin_tx,0);
			}
		}
	}

	if (of_property_read_u32(pdev->dev.of_node,"pin_rx",&cdns_uart_data->pin_rx)){
		printk(KERN_EMERG "UART:%d, : No pin_rx defined in dts",cdns_uart_data->id);
	} else {
		sprintf(gname,"uart%d-pin-rx",cdns_uart_data->id);
		if (!gpio_is_valid(cdns_uart_data->pin_rx)){
			printk(KERN_EMERG "Invaild RX PIN \n");
		} else {
			gpio_request(cdns_uart_data->pin_rx,gname);
			gpio_direction_output(cdns_uart_data->pin_rx,0);
		}
	}


	printk(KERN_EMERG "xlnx_psuart:%d:pin_tx=%d,pin_rx=%d,txempty=%d,channel type=%d\n",\
			cdns_uart_data->id,cdns_uart_data->pin_tx,\
			cdns_uart_data->pin_rx,cdns_uart_data->txempty,\
			cdns_uart_data->type);
	...
	cdns_uart_data->clk_rate = CDNS_UART_REFSPEED;
...
static int cdns_uart_remove(struct platform_device *pdev)
{
	...
	gpio_free(cdns_uart_data->pin_tx);
	gpio_free(cdns_uart_data->pin_rx);
